...
const LatestTxsItem = ({ tx, isLoading }: Props) => {
  ...
  const protocolTag = tx.to?.metadata?.tags?.find(
    (tag) => tag.tagType === "protocol",
  );
  ...
  return (
    <Grid
      gridTemplateColumns={{
        lg:
          columnNum === 2
            ? "3fr minmax(auto, 200px)"
            : "3fr minmax(auto, 200px) 170px",
        xl:
          columnNum === 2
    ...
    >
      <Flex overflow="hidden" w="100%">
        ...
        <IconSvg name="txn_batches" boxSize={6} color="text.secondary" />
